...
<div class="row">
    <div class="col-md-8">
        ...
        <form class="shadow-sm bg-white p-3" action="{{route('orders.update', [$order->id])}}" method="POST">
            ...
            <label for="invoice_number">Transaksi number</label><br>
            ...
            <label for="">Barang ({{$order->totalQuantity}}) </label><br>
            <ul>
                @foreach($order->barang as $item)
                <li>{{$item->nama}} <b>({{$item->pivot->quantity}})</b></li>
                @endforeach
            </ul>
